...
- [x] **AC-2** — Live-smoke records one multi-turn conversation with ≥1 MCP tool; verify node kinds + usage extraction. *Closed in R85 (v0.7.0 GA-gate): `scripts/dogfood/arc_b_slice_3_mcp.py` + `tests/live/test_anthropic_agents_mcp_smoke.py` drive the real OneAPI relay (`Claude Sonnet 4.6`) end-to-end with an in-process SDK MCP server (`create_sdk_mcp_server`, no Node.js subprocess required). Verified invariants: run.status=COMPLETED, AssistantMessage(ToolUseBlock) recorded with `state_after['tool_use_id']`, UserMessage(ToolResultBlock) recorded with matching `tool_use_id` (R76 ADR-026 §5.1 linkage contract), and final TextBlock contains the tool result. **Note**: ToolUseBlock/ToolResultBlock currently surface as `kind=NodeKind.LLM` (kind comes from message type, not block type) — the `NodeKind.TOOL` entry in the recorder block-dispatch table is unused for these and **was reconciled docs-only at R89** via [`docs/contracts/adapter-protocol.md`](../contracts/adapter-protocol.md#envelope-determines-kind-r85-contract-finding-codified-at-r89): the envelope-determines-kind rule is now the documented cross-adapter contract, the dead `ToolUseBlock`/`ToolResultBlock` map entries are documented as defensive forward-compat fallbacks, and `docs/adapters/anthropic_agents.md`'s mapping table was fixed (`UserMessage` was incorrectly listed as `kind=fn` / `name=user`; corrected to `kind=llm` / `name=UserMessage`).*
- [x] **AC-3** — Fork primitive re-invokes agent from `at_node_id` with `overrides`; live-smoke green. *Closed in R87 (v0.7.0 GA-gate): real-relay override-fork live-smoke turned green this round. The R86-shipped dogfood `scripts/dogfood/arc_b_slice_3_fork_override.py` + pytest wrapper `tests/live/test_anthropic_agents_fork_override_smoke.py` ran end-to-end against the OneAPI relay (`Claude Sonnet 4.6`) and exited 0 with the `AC-3 release-gate INVARIANTS GREEN` marker. Concrete evidence: parent `run_id=e60c8692-4a30-440d-8333-23e09e93f366` recorded `parent_tu_id=toolu_bdrk_01NRJ958p1qAFNtSfNEuLXBU` for operands 4127+8956=13083; `recorder.fork(parent, anchor, tool_input_overrides={parent_tu_id: {a: 100, b: 200}})` returned `child_run_id=206b9e0a-ce8a-4853-ab1b-5e451aced6f5` and `fork_id=7b6d2b9c-6b1e-4cd0-b48d-88ac663202c9`; child run completed with a fresh SDK-minted `child_tu_id=toolu_bdrk_01JFteNbHxtsitAd8yXosj3E` (≠ parent's, INV-5 asserted positively), matching `ToolUseBlock` ↔ `ToolResultBlock` linkage on that fresh id (R76 ADR-026 §5.1 contract holds across fork), and the child's final `TextBlock` contained the override-sum `300` (proving the `{a:100, b:200}` override surfaced user-side via `resume=child_sid` continuation per R86 contract finding). pytest live wrapper `tests/live/test_anthropic_agents_fork_override_smoke.py` `1 passed in 54.08s` with `CHRONOS_LIVE=1`. The R86 contract pre-finding (immediately below) is therefore promoted from "pre-finding" to "finding" — observed against the real relay, exactly as predicted from source-inspection.* `tool_input_overrides` validates the parent `tool_use_id` is a matched use-side key, then delegates verbatim to `claude_agent_sdk.fork_session(session_id, up_to_message_id=uuid)`. The SDK fork is a pure JSONL rewrite of the parent transcript up to fork point — it does NOT splice the override into the child transcript. The override surfaces user-side via the continuation prompt under `resume=child_sid`. The child's ToolUseBlock therefore carries a *fresh* SDK-minted `tu_id`, NOT the parent's; consequently `state_after['tool_input']` is NOT stamped on the child's AssistantMessage node (the recorder stamps only on id-match). This is symmetric to R64's "Identity fork ≠ byte-identical trace" finding for LangGraph and is documented at the dogfood module-doc + asserted positively as INV-5. The R80 fake-SDK unit tests assert the stamp because the fixture echoes the parent tu_id verbatim — that is a fixture artifact of the offline test path, NOT a real-relay contract. (When the live-smoke does land, this paragraph will be promoted from "pre-finding" to "finding".)*
...
**GA-gate update (R85)**: AC-2 promoted partial→full. The MCP live-smoke gap was unblocked by switching from "out-of-process MCP server via Node.js subprocess" to `claude_agent_sdk.create_sdk_mcp_server` (in-process Python tool, zero subprocess). Outstanding GA-gate work: AC-3 partial → full (real-relay override-fork live-smoke).

**GA-gate verdict (R87, GREEN — v0.7.0 GA cut)**: AC-3 promoted partial→full. With AC-1, AC-2, AC-4, AC-5 already at `[x]`, all five Arc B slice 1 release gates are now closed. Real-relay override-fork live-smoke observed green this round (see AC-3 evidence above); the R86 SDK-masked relay-error envelope was a transient OneAPI relay flake, not a structural blocker, and the dogfood degradation classifier hardening from R86 (`scripts/dogfood/_degradation.py` + `tests/unit/test_dogfood_degradation.py`) means future flakes won't be mis-classified as hard regressions. **R87 honors the R86 honesty rule**: AC-3 was flipped `[x]` only after the live-smoke marker was actually observed in this round's output — no pre-emptive flips, no aspirational release blocks. The `chronos-release-pattern` skill's "disprover-first" rule applied here too: the cheap R85 MCP probe (re-running today's relay state, $0.05) was used as the gating disprover before committing budget to the more expensive R86 fork-override re-run ($0.14); both passed → green path. v0.7.0 GA cut proceeds in this same round per the `chronos-release-pattern` 8-step sequence.
