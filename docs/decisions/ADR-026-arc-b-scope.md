...
**GA-gate verdict (R86, deferred)**: AC-3 attempted but **not promoted** — real-relay override-fork live-smoke implementation landed (`scripts/dogfood/arc_b_slice_3_fork_override.py` + `tests/live/test_anthropic_agents_fork_override_smoke.py`), but the OneAPI relay is currently in synthetic-auth-failed mode (R69 spike #3.4 redux); both the R85 MCP smoke and the R86 fork-override smoke exit 2 (relay degraded) today. AC-3 stays `[~]`. AC-2 stays `[x]` on the strength of R85's recorded INVARIANTS-GREEN run (a relay flake at R86 does not retroactively unratchet R85's recorded fact). **v0.7.0 GA cut deferred** — R87 will decide between (a) wait-for-relay-recovery and re-attempt the live smoke or (b) author a deterministic offline-fixture-based AC-3 closure path so the GA gate is no longer relay-dependent. The R86 round also hardened the dogfood degradation classifier against SDK-masked relay-error envelopes (now treats `"Claude code returned an error result"` as relay-degraded, not hard regression — see CHANGELOG `[Unreleased]`).
