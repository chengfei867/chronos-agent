...
**截至 Round 85 结束 (2026-05-18 CST ~07:00-08:00 + ~10:00 cron slot — 2-slot impl round, A2 close-out #11 per `cron-slot-handoff-recovery`; well inside 0–11 窗口) — Phase 4 Arc B GA-gate AC-2 fully closed. R85 = 真 GA-gate close round: prior cron slot (~07:00) 跑了 30-line probe 推翻 R83 \"AC-2 needs Node.js subprocess + MCP server fixture\" 的 deferral 假设 — 发现 `claude_agent_sdk.create_sdk_mcp_server` ships in-process Python MCP server (no `npx`, no Node, no subprocess), 写了 `scripts/dogfood/arc_b_slice_3_mcp.py` (~280 LOC, 1-tool `add(a,b)→int` + 5 runtime invariants: run.status=COMPLETED / AssistantMessage(ToolUseBlock) with `state_after['tool_use_id']` / UserMessage(ToolResultBlock) with matching id / R76 linkage contract / final TextBlock 含 sum 容忍 thousands-separator) + `tests/live/test_anthropic_agents_mcp_smoke.py` (~80 LOC pytest wrapper, `CHRONOS_LIVE=1` + `ANTHROPIC_API_KEY` gated, marker `@pytest.mark.live`, asserts dogfood exit 0 + INVARIANTS-GREEN marker), 跑通 (`Claude Sonnet 4.6` via OneAPI relay, ~7s wallclock, ~$0.14 cost) → ADR-026 §6 AC-2 promoted `[~]` → `[x]` (in-place per R57) + GA-gate update line narrowing剩余 GA-blocking work to AC-3 only + recorder contract finding documented inline (ToolUseBlock/ToolResultBlock surface as `kind=NodeKind.LLM` because recorder stamps kind from message-type dispatch not block-type — `recorder.py:77` `NodeKind.TOOL` entry unused for these blocks; `tool_use_id` IS in `state_after` so AC-2 unaffected, reconciliation deferred R86 candidate) + CHANGELOG `[Unreleased]` R85 Added/Changed/Quality-bar blocks + 完整 progress doc 写在 commit-pending 状态. 但 prior slot ran out of budget before CONTEXT §5/§6 refresh + commit + push + QQ — this slot (~10:00) executed standard A2 close-out: `git fetch origin main` confirmed in sync, `git status` 5 paths uncommitted (2 modified md + 3 new files) matching progress-doc \"Files touched\" section, gates re-verified green (631/8/0/0 in 18.03s — +1 skipped vs R84's 7-skipped is exactly the new live-smoke), `git diff pyproject.toml` empty (no lockfile trap), ruff check + format + mypy 全 clean, then patched CONTEXT §5/§6 + footer + commit + push (gh-proxy) + QQ. Adapter-1-3 zero-regression streak: R52→R85 = **33 rounds** (新 project-history high; R85 only touched `scripts/dogfood/` + `tests/live/` + docs, zero `src/` change). 无 tag (`[Unreleased]` 继续累积至 v0.7.0 GA, gating now solely on AC-3). R86 候选 1 (推荐): AC-3 GA-gate close — real-relay override-fork live-smoke (R85-style: probe assumption first, full close if path is clear) — last GA-blocking checkbox 关掉就可以 cut v0.7.0 GA.**
...
**Round 86 — AC-3 GA-gate close (推荐) / 第 6 个 site fixture migrate (env-blocked fallback) / Recorder kind contract reconciliation (双重 blocked fallback); 1-slot pre-budget — 但 R85 已 5-round 验证 Arc B impl rounds 倾向 2-slot, 故 first slot 优先 probe + dogfood + invariants, second slot 收尾如需要**

战略视角: R85 关闭了 GA-blocking AC-2 (real-relay MCP-tool live-smoke + 5 runtime invariants + in-process SDK MCP server bypass of Node.js subprocess assumption). Alpha v0.7.0a2 已 ship 在 R83; 现 GA gate (`v0.7.0`) 只剩 **AC-3 partial → full** (real-relay override-fork live-smoke). R86 攻这条 path 后即可 cut v0.7.0 GA. R85 的 \"probe-deferral-assumption-with-30-line-spike\" 和 \"create_sdk_mcp_server in-process\" 经验是 R86 的强预设 — AC-3 的 path 非常可能比 R83 originally 假设的 \"Node + MCP server + relay\" 简单.

### Option A (推荐, 1-2 slot): GA-gate AC-3 partial → full tick (real-relay override-fork live-smoke)

- **Background**: ADR-026 §6 AC-3 现 `[~]` partial — fork-without-overrides 有 R74 real-relay live-smoke; fork-with-overrides 只有 fake-SDK dogfood + unit tests (R80 tool-input override, R82 tool-result override). GA gate 要求 real relay against `fork_session(at_node_id, overrides={...})`.
- **P0 dispatch (slot-1)**:
  1. **Probe first** (R85 invariant): 30-line spike — 复用 R85 `arc_b_slice_3_mcp.py` 的 setup, 跑 record + identify ToolUseBlock node + `recorder.fork_session(at_node_id, tool_input_overrides={tu_id: {a: 1, b: 2}})` against live OneAPI relay. 验证 SDK fork-with-override semantics 是否 cleanly 走 OneAPI relay (R85 验证了 ToolUseBlock 协议端到端工作, 故 fork 可能也行). 若可行 → P1 直接写 dogfood; 若不可行 → 记 finding 写 ADR + escalate (env / SDK API blocker).
  2. 写 `scripts/dogfood/arc_b_slice_3_fork_override.py` 或 extend R85 dogfood (沿用 in-process SDK MCP server pattern): record initial run → identify ToolUseBlock node id → fork with `tool_input_overrides={tu_id: {a: 100, b: 200}}` → 验证 forked run 收到 `add(100, 200)=300` (而非原 4127+8956=13083) → final TextBlock 含 \"300\" — 5 invariants like R85: forked-run.status=COMPLETED / forked AssistantMessage(ToolUseBlock).input == override / forked UserMessage(ToolResultBlock).content matches new sum / forked TextBlock contains override sum / parent run untouched.
  3. 写 pytest live wrapper `tests/live/test_anthropic_agents_fork_override_smoke.py` (mirror R85 wrapper: `CHRONOS_LIVE=1 + ANTHROPIC_API_KEY` gate, marker `@pytest.mark.live`, subprocess-runs dogfood, asserts exit 0 + INVARIANTS-GREEN marker).
  4. 跑通 → ADR-026 §6 AC-3 `[~]` → `[x]` in-place per R57 + closing note + GA-gate verdict update line ("AC-1..AC-5 全 ticked, GA gate 满足").
- **P1 (slot-1 or slot-2)**: CHANGELOG `[Unreleased]` R86 Added/Changed/Quality-bar blocks. progress doc. CONTEXT §5/§6 + footer refresh.
- **P2 (slot-2 if budget允许)**: cut **v0.7.0** GA — 3 个 version files bump (`pyproject.toml`, `src/chronos/__init__.py`, `src/chronos/cli/__init__.py`) + uv.lock + roll `[Unreleased]` → `[0.7.0] — 2026-05-?? (R71-R86 bundle)` + 写 release notes (highlights / install / caveats / migration) + git tag annotated + push tag + GitHub Release page (prerelease=false, make_latest=true 接 v0.6.0). 跟 `chronos-release-pattern` skill.
- Gate expect: 631/9/0/0 (R85 baseline 631/8/0/0 + 1 new live-smoke skipped by default). Adapter-1-3 streak R52→R86 = **34 rounds**.
- **Hard blockers (escalate to user)**: SDK fork-with-override semantics 不通 OneAPI relay (probe 阶段会发现) / `ANTHROPIC_API_KEY` 没配 / OneAPI 余额耗尽.

### Option B (env-blocked fallback, 1 slot): migrate 第 6 个 fixture site

- **Background**: R84 故意 defer `tests/unit/test_adapter_anthropic_agents.py` — richer `_StubBlockBase` shape (`is_error`/`thinking`/`signature`) + runtime `_blk(cls_name, **kw)` factory. R84 已在 `tests/unit/fixtures/anthropic_agents_stubs.py` 导出 `make_block` / `make_message` factories.
- **P0 dispatch**: 决策 `is_error`/`thinking`/`signature` 命运 — (a) 加到共享 `StubBlock` (R64 invariant: 不预先添加未消费字段, audit 5 个 site 是否实际消费); 或 (b) 保留 file-local 子类 `_StubBlockExtra(StubBlock)` 加 3 字段, 共享 surface 不变. Migrate `test_adapter_anthropic_agents.py`. 跑 baseline 不动.
- Gate expect: 631/8/0/0 unchanged. Streak R52→R86 = 34 rounds.

### Option C (双重 blocked fallback, 0.5 slot): Recorder kind dispatch contract reconciliation

- **Background**: R85 contract finding — recorder stamps `NodeKind` from message-type dispatch (`type(msg).__name__`, recorder.py:27-46), 不是 block-type dispatch. ToolUseBlock/ToolResultBlock 在 AssistantMessage/UserMessage 里时 node 仍 `kind=LLM`. `recorder.py:77` 的 `\"ToolUseBlock\": NodeKind.TOOL` 表项 unused.
- **P0 dispatch**: 决策 (a) 把 \"envelope-determines-kind\" 写进 AdapterProtocol contract docs (轻量) OR (b) 写 ADR-027 拟定 \"split per-block nodes\" refactor (ADR-027 Draft + spike + 评估 N倍 node count blowup vs query DX). R86 可能只到 (a) — (b) 是更大单元.
- Tracked as low-priority since 不 block GA, AC-2 已用 `state_after` shape 关闭.

### Option D (post-GA polish, 0.5 slot): R85 dogfood hardening

- INV-4 当前 asserts text contains literal sum; 可强化为 extract `13083` from `state_after['blocks'][i]['content'][0]['text']` of ToolResultBlock node and compare to expected (gate against model returning wrong number from tool). Optional.

### 推荐: Option A (AC-3 GA-gate close)

理由:
1. v0.7.0 GA cut 是当前**唯一**最大目标 — AC-3 是最后 GA-blocking checkbox. 关掉就可以 cut tag.
2. R85 经验: probe-first-on-deferral-assumption 10 分钟成本可能直接 unblock multi-round 假设. AC-3 deferral note 也是 \"requires real relay infra\" — R85 已证明 OneAPI relay 通端到端 ToolUse + ToolResult, fork-with-override 极可能也通.
3. AC-3 跟 AC-2 共享大量 setup (in-process SDK MCP server, 5-invariant pattern, live-smoke wrapper) — R85 dogfood 是 R86 dogfood 的 80% template.
4. R85 的 \"dogfood-as-release-gate 直接关 GA-blocker\" pattern (R64 invariant 实战应用) 可直接复用.

如果 Option A probe 5 分钟内告失败 → 立即切 Option B 不浪费 slot.

### R86 hand-off invariants

- Window: 0–11 CST (cron) or manual chat slot.
- Baseline (post-R85): **631 pass / 8 skip (live, including new R85 MCP smoke) / 0 xfail / 0 fail**, mypy 0 (38 src files), ruff clean.
- Open command: `git fetch origin main && git pull --ff-only` + `uv run pytest -q --no-cov` (expected 631/8/0/0) + `git tag --list \"v0.7.0*\"` (expected: v0.7.0a1 + v0.7.0a2, no GA yet) + 读 R85 progress doc + ADR-026 §6 verdict (AC-2 现 `[x]`, AC-3 仍 `[~]`) + (Option A 才跑) `echo $ANTHROPIC_API_KEY | head -c 20` 验证 env.
- Adapter-1-3 streak now **33 rounds** (R85 new high); R86 必须不破.
- R85 invariants 上墙 (复用 for R86):
  - **`create_sdk_mcp_server` = in-process Python MCP server**, no Node.js subprocess.
  - **Recorder kind from message-type, not block-type** — ToolUseBlock 在 AssistantMessage 时 node `kind=LLM`; `state_after['blocks'][i]['block']` (key 是 `'block'`, 不是 `'type'`) 是检测 block-type 的方式.
  - **`tool_use_id` 在 `state_after`** 完整可恢复, R76 linkage contract holds.
  - **Probe deferral-assumption with 30-line spike before budgeting multi-round work** (R85 unblocked AC-2 一个 round 而非 R83 假设的 multi-round).
  - **Dogfood-as-release-gate 直接关 GA-blocker checkbox** (R85 应用, R64 invariant 实战升级): pytest wrapper subprocess-runs dogfood + greps INVARIANTS-GREEN marker = belt-and-suspenders against criterion drift.
- Inherited dogfoods (DO NOT delete): `arc_b_slice_1_smoke.py`, `arc_b_slice_2_fork.py`, `dogfood_fork_tool_override.py`, `dogfood_fork_tool_result_override.py`, `arc_b_slice_3_mcp.py` (R85 new). 全部 exit 0 baseline.
- `[Unreleased]` 现含 R85 Added + Changed + Quality bar blocks. R86 (Option A success) 在其下追加 + 若 cut GA 则 roll `[Unreleased]` → `[0.7.0]`.
- Live-smoke files now 4 in `tests/live/`: `test_anthropic_agents_smoke.py` (R71), `test_anthropic_agents_fork_smoke.py` (R74), `test_anthropic_agents_mcp_smoke.py` (R85), `test_real_llm_smoke.py` + `test_crewai_smoke.py` (Phase 3). R86 Option A 加第 5 个: `test_anthropic_agents_fork_override_smoke.py`.

### Round 85 (上一轮) 现状

✅ R85 已结. AC-2 GA-gate full-tick closed via real-relay MCP-tool live-smoke (`scripts/dogfood/arc_b_slice_3_mcp.py` + `tests/live/test_anthropic_agents_mcp_smoke.py`), in-process SDK MCP server bypass of Node.js subprocess. 5 runtime invariants gate the dogfood. Recorder kind-dispatch contract finding documented (ToolUseBlock surfaces as `kind=LLM`; `state_after['blocks'][i]['block']` is the block-type key) — 不是 bug, 是 contract clarity gap, R86 Option C 候选. Adapter-1-3 streak 33 rounds. 631/8/0/0 baseline (+1 skipped vs R84 = new live-smoke). Cost ~$0.14. 详见 `docs/progress/2026-05-18-round-85.md`.
...
- **R85 (GA-gate AC-2 close, no tag) — `scripts/dogfood/arc_b_slice_3_mcp.py` + `tests/live/test_anthropic_agents_mcp_smoke.py` real-relay MCP live-smoke; ADR-026 §6 AC-2 `[~]` → `[x]`** ← **this round**
- v0.7.0 🚧 target ~R86-R87 GA — only AC-3 (override-fork live-smoke against real relay) partial-tick remaining; R86 推荐 attack AC-3 (R85-style probe-first).
