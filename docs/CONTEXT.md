...
**Round 87 — AC-3 closure path decision (Option α wait-for-relay vs Option β offline-fixture); 1-2 slot pre-budget — 优先 30-line relay-probe (R85 invariant) at slot start to choose path cheaply**

战略视角: R86 attempted AC-3 close 但被 OneAPI relay flake 阻止. R86 prior slot 已经写了 production-grade dogfood `arc_b_slice_3_fork_override.py` + pytest wrapper + extracted classifier `_degradation.py` + 17-case unit test — **scaffolding 全已 in place**, R87 只需要 (a) 在 green-relay 窗口 re-run 取得 INVARIANTS-GREEN ratchet, OR (b) 走 offline-fixture path 解耦 GA gate 跟 relay 健康. R86 的 relay-flake corollary 也已上墙: AC-2 stays `[x]` 即使今天 R85 dogfood 也 exit 2 — relay flake 不 retroactively unratchet adapter-code claims. R87 first action **必须**是 30-line relay probe (~$0.05) 决定 path 而不是直接重跑 R86 dogfood (~$0.14 each, 3-4 attempts).

### Option A (推荐 path-A1, 0.5-1 slot): probe relay → if green, re-run R86 dogfood → cut v0.7.0 GA

- **Background**: R86 production dogfood + wrapper + classifier + unit test 全部已 ship 在 R86 commit 里. AC-3 stays `[~]` only because the live-run was relay-flaked. 不需要任何新代码 — 只需要 green relay window.
- **P0 dispatch (first 5 minutes)**: 30-line probe spike 跑 R85 dogfood `arc_b_slice_3_mcp.py` 一次 (~$0.05 if green, exit 2 if degraded). 这同时也是 AC-2 ratchet sanity check. 如果 exit 0 → relay 已恢复, 切 P1 立即. 如果 exit 2 → relay 仍 degraded, 切 Option B (offline-fixture).
- **P1 (slot-1 if relay green)**: 跑 R86 dogfood `scripts/dogfood/arc_b_slice_3_fork_override.py` (~$0.14). 5 invariants pass + INVARIANTS-GREEN marker → ADR-026 §6 AC-3 `[~]` → `[x]` in-place per R57 + GA-gate verdict line "R87, GREEN" + closing note (cite R86 contract pre-finding paragraph: SDK fork-with-tool_input_overrides delegates to `fork_session(up_to_message_id=uuid)`, child carries fresh tu_id, child's `state_after['tool_input']` NOT stamped — symmetric to R64 LangGraph identity-fork-≠-byte-identical-trace, R86 calls this "pre-finding" and R87 promotes to "finding").
- **P2 (slot-1 or slot-2)**: cut **v0.7.0** GA per `chronos-release-pattern` skill — 3 个 version files bump (`pyproject.toml`, `src/chronos/__init__.py`, `src/chronos/cli/__init__.py`) `0.7.0a2` → `0.7.0` + `uv lock --offline` + roll `[Unreleased]` → `[0.7.0] — 2026-05-?? (R71-R87 bundle)` + 写 release notes (highlights / install / caveats / migration; bundle R74-R86 全部 R71-R87 成果 = real record + fork + override-fork pipeline + GA-gate AC-1..5 全 ticked) + git tag annotated v0.7.0 + push tag + GitHub Release page (prerelease=false, make_latest=true 接 v0.6.0). 跟 `chronos-release-pattern` skill all 8 steps.
- Gate expect: 648/9/0/0 (no test count delta from R86; live-smoke runs only with CHRONOS_LIVE=1). Adapter-1-3 streak R52→R87 = **35 rounds**.
- **Hard blockers (escalate to user)**: relay 长期 degraded (>2 rounds, 即 R88+ 仍 exit 2) → 必须切 Option β; `ANTHROPIC_API_KEY` 没配 / OneAPI 余额耗尽 → 直接 escalate.

### Option B (推荐 path-A2 if relay degraded, 1-2 slot): offline-fixture-based AC-3 closure

- **Background**: R86 finding F4 — AC-3 currently 完全 relay-coupled, 任何 relay flake block GA. Long-term solution: 解耦 AC-3 gate 跟 environmental dependencies. **强 R87 候选 if relay-probe exit 2**.
- **P0 dispatch**: 设计 `tests/fakes/anthropic_agents_session_replay.py` shape — fake SDK that emits `Message` types from a pre-recorded JSONL transcript. 设计 fixture capture protocol (one-time, requires green-relay window — 现已不可达, 但 R85 的 `arc_b_slice_3_mcp.py` ratchet 已 capture 完整 Multi-turn MCP transcript on 2026-05-18 17.58s wallclock, 可能还有 SQLite store 残骸 — `find` 一下).
- **P1**: 写 `tests/spikes/spike14_offline_session_replay.py` 30-line spike 验证 fake-SDK 接受 R85 captured transcript JSONL + 重放出 SDK 期待的 Message types 给 recorder → recorder 正确处理 → query layer 正确 build node tree.
- **P2**: 升级 R86 dogfood `arc_b_slice_3_fork_override.py` 接受 `--offline-fixture` flag, env var `CHRONOS_DOGFOOD_OFFLINE_FIXTURE=...path/to/jsonl`, 走 fake-SDK 而非 OneAPI relay. 5 invariants 不变. exit 0 in offline mode + INVARIANTS-GREEN marker → AC-3 closure path independent of relay health.
- **P3**: ADR-027 (新, Draft → Accepted in same round 走 R57 in-place) "AC-3 offline-fixture closure path" 写明: rationale (relay-coupling design issue, R86 F4) + protocol (capture green-relay JSONL → replay through fake → assert recorder + fork primitive correctness on captured transcripts) + acceptance criteria (≥1 captured fixture under `tests/fixtures/anthropic_agents_session_replay/`, ≥3 invariants asserted on each fixture) + relationship to existing ADR-026 §6 AC-3 (offline-fixture is the **primary** closure path; live-relay is now optional belt-and-suspenders).
- Gate expect: 648-650/9/0/0 (+ 0-2 new offline-fixture unit tests, 1 spike file). Adapter-1-3 streak unchanged.
- **Risk**: fixture capture currently impossible if R85 store已被 cleanup. Fallback: wait for relay window even briefly, capture once, then never depend on relay again for AC-3.

### Option C (post-GA polish, 0.5 slot): R85/R86 dogfood hardening + recorder kind contract reconciliation

- **Background**: R85 contract finding (recorder stamps `NodeKind` from message-type, not block-type — `recorder.py:77` `NodeKind.TOOL` entry unused for ToolUseBlock-in-AssistantMessage cases). 不 block GA, 但 contract clarity gap. R86 已 documented in ADR-026 §6 AC-2 closing note. R87 可在 GA cut 后顺手 close.
- **P0 dispatch**: 决策 (a) 把 "envelope-determines-kind" 写进 AdapterProtocol contract docs (轻量) OR (b) 写 ADR-027 (or 028 if Option B took 027) 拟定 "split per-block nodes" refactor. R87 推荐 (a) — pure docs, ~30 LOC delta, 不 risk regression. (b) 是大单元, 留给 Phase 5 if proven需要.
- Tracked as low-priority since 不 block GA, AC-2 + AC-3 (一旦 closed) 都用 `state_after` shape 工作.

### Option D (env-blocked fallback only, 0.5 slot): migrate 第 6 个 fixture site (R84 deferred)

- **Background**: R84 故意 defer `tests/unit/test_adapter_anthropic_agents.py` migration — richer `_StubBlockBase` shape (`is_error`/`thinking`/`signature`) + runtime `_blk(cls_name, **kw)` factory shape. R84 已在 `tests/unit/fixtures/anthropic_agents_stubs.py` 导出 `make_block` / `make_message` factories.
- 只在 Option A 和 Option B 都 blocked 时考虑 — 是 *真* 的 fallback, 不该挤占 R87 主路径.

### 推荐: Option A (probe-first), 切到 Option B (offline-fixture) if probe exit 2

理由:
1. R86 已经 ship 全部 production-grade scaffolding (dogfood + wrapper + classifier + 17 unit tests + ADR pre-finding). R87 不需要写新代码就能 close AC-3 — 只需要 relay 健康一次.
2. 30-line probe 5 分钟成本 ($0.05) 决定 path A vs B,避免 R87 走 wrong path 浪费整 slot.
3. Option B (offline-fixture) 是 *long-term right move* 即使 Option A 成功也值得 R88-R90 做 — AC-3 应该 deterministic, 不该 forever depend on relay 健康.
4. v0.7.0 GA cut 是 当前 R86 之外**唯一**最大目标 — Option A path 能 close, Option B path 也能 close.
5. R86 honesty rule (R86 F1 上墙): 如果 R87 也 hit relay flake, **不要重蹈 R86 prior slot 的 aspirational-release-doc trap** — re-run live, observe ground truth, only flip ACs `[x]` after observed green. R86 把 trap 写进了 skill, R87 必须 honor it.

如果 30-line probe 30 秒内告 relay 仍 degraded → 立即切 Option B 不浪费 slot.

### R87 hand-off invariants

- Window: 0–11 CST (cron) or manual chat slot.
- Baseline (post-R86): **648 pass / 9 skip (live, including R85 MCP smoke + R86 fork-override smoke) / 0 xfail / 0 fail**, mypy 0 (38 src files), ruff clean, format clean, lockfile clean.
- Open command: `git fetch origin main && git pull --ff-only` + `uv run pytest -q --no-cov` (expected 648/9/0/0) + `git tag --list "v0.7.0*"` (expected: v0.7.0a1 + v0.7.0a2, no GA yet) + 读 R86 progress doc + ADR-026 §6 verdict (AC-2 现 `[x]`, AC-3 仍 `[~]` w/ GA-gate verdict R86 deferred line) + (Option A 才跑) `echo $ANTHROPIC_API_KEY | head -c 20` 验证 env.
- Adapter-1-3 streak now **34 rounds** (R52→R86); R87 必须不破.
- R86 invariants 上墙 (R87 必须 honor):
  - **Aspirational-release-doc trap**: 不写 release block 在 live-smoke 实际 green 之前. 不 flip AC `[x]` 在 INVARIANTS-GREEN marker 实际观察到之前. (Codified in `cron-slot-handoff-recovery` skill.)
  - **Relay-flake corollary**: 历史 ratchet `[x]` 在 relay flake 时不 unratchet. AC-2 stays `[x]` even though R85 dogfood today exits 2. AC-3 stays `[~]` until *observed* green, not because R86 was confidence-degraded.
  - **`is_relay_degraded_exception(exc) -> bool`**: 任何 future dogfood 跑 OneAPI relay 必须 import + use this classifier — 不要重蹈 R85 inline 3-marker mistake. 添加新 envelope shape: 在 `_degradation.py` 加 substring + 在 `test_dogfood_degradation.py` 加 case + 跑 17→18 unit tests.
  - **Spike disposal rule**: 一次性 scaffolding spike, 学习 encoded into production artifacts → delete (R86 chose for `_r86_probe.py`). 多 round-relevant contract probe → `tests/spikes/spikeN_*.py` (per `chronos-spike-authoring`). 默认 delete unless 明显 reusable.
  - **2-slot pre-budget for impl rounds** + **A2 inheritance can include prior-slot-honest-revert** — slot-1 may revert its own aspirational claims before dying; slot-2 verifies-and-ships, not redo.
- Inherited dogfoods (DO NOT delete): `arc_b_slice_1_smoke.py`, `arc_b_slice_2_fork.py`, `dogfood_fork_tool_override.py`, `dogfood_fork_tool_result_override.py`, `arc_b_slice_3_mcp.py` (R85), **`arc_b_slice_3_fork_override.py` (R86 new)**, `_degradation.py` (R86 shared classifier — not a dogfood, support module). 全部已 commit; AC-2 + AC-3 dogfoods exit 2 today against degraded relay (do NOT confuse w/ regression).
- `[Unreleased]` 现含 R85 + R86 entries. R87 (Option A success) 在其下追加 + roll → `[0.7.0]`. R87 (Option B success) 在其下追加 ADR-027 + offline-fixture entries; tag可能仍延后 if Option B alone 不够 ratchet AC-3 `[x]`.
- Live-smoke files now 5 in `tests/live/`: `test_anthropic_agents_smoke.py` (R71), `test_anthropic_agents_fork_smoke.py` (R74), `test_anthropic_agents_mcp_smoke.py` (R85), `test_anthropic_agents_fork_override_smoke.py` (R86 new), `test_real_llm_smoke.py` + `test_crewai_smoke.py` (Phase 3) — R87 不增加 unless Option B 走 offline-fixture variant.
- Ratcheting summary (cumulative pre-R87):
  - Adapter-1-3 zero-regression streak: 34 rounds (R52 → R86).
  - A2 inheritance chain: 12 rounds (R48-A → … → R86).
  - Dogfood-as-release-gate count: 6 (4 fake-SDK from R71/R74/R80/R82 + 2 real-relay from R85/R86).
  - GA-blocker checkboxes outstanding: AC-3 only (down from 5 at R71 entry).
  - Skill updates this session: `cron-slot-handoff-recovery` (R86 aspirational-release-doc trap section + relay-flake corollary).

### Round 86 (上一轮) 现状

✅ R86 已结. AC-3 GA-gate **NOT closed** (relay degradation, env-flake, deferral conservative); v0.7.0 GA cut **deferred to R87+**. R86 prior slot 写了 AC-3 release-gate dogfood + wrapper + extracted classifier + 17-case unit test, 跑 hit relay-degraded, 写了 prospective `[0.7.0]` GA block, 又自己 reverted to `[Unreleased]` honest findings. 本 slot 验证 prior slot revert 诚实 (gates 648/9/0/0 match), 删了 scratch spike `_r86_probe.py`, 写了 progress doc, refresh CONTEXT, commit + push. **新 failure-mode "aspirational-release-doc trap" codified into `cron-slot-handoff-recovery` skill**. **Relay-flake corollary: AC-2 stays `[x]` on R85 ratchet 即使 R85 dogfood 今天也 exit 2** — env flake 不 retroactively unratchet adapter claims. 详见 `docs/progress/2026-05-19-round-86.md`.
...
- R84 (cleanup, no tag) — `tests/unit/fixtures/anthropic_agents_stubs.py` extraction (5 of 6 sites migrated; pure refactor)
- R85 (GA-gate AC-2 close, no tag) — `scripts/dogfood/arc_b_slice_3_mcp.py` + `tests/live/test_anthropic_agents_mcp_smoke.py` real-relay MCP live-smoke; ADR-026 §6 AC-2 `[~]` → `[x]`
- **R86 (GA-gate AC-3 attempt, NOT closed; aspirational-release-doc trap discovery + recovery; classifier hardening; no tag) — `scripts/dogfood/_degradation.py` + `scripts/dogfood/arc_b_slice_3_fork_override.py` + `tests/live/test_anthropic_agents_fork_override_smoke.py` + `tests/unit/test_dogfood_degradation.py`; ADR-026 §6 AC-3 stays `[~]`; GA cut deferred** ← **this round**
- v0.7.0 🚧 target ~R87-R88 GA — only AC-3 (override-fork live-smoke) partial-tick remaining; R87 推荐 probe-first then either re-run R86 dogfood (Option A) OR offline-fixture closure path (Option B, ADR-027 candidate).
