...
**Round 88 — Phase 4 Arc B GA shipped; multiple low-risk polish + relay-independence candidates available; 1-2 slot pre-budget per option**

Phase 4 Arc B slice 1 GA closed at R87. v0.7.0 GA tag is live. There is **no GA-blocking work** for R88. Below are the post-GA candidates ranked by R87 hand-off; pick one based on relay availability, fresh user steering (if any in `/workspace/.hermes/.env` or QQ inbox), and slot budget. R87 default recommendation: **Option β** (offline-fixture AC-3 closure path) — un-couples GA-gate from relay, becomes ADR-027 candidate, removes recurring relay-flake risk for v0.7.x patch series.

### Option β — offline-fixture AC-3 closure path (default, ADR-027 candidate, **1-2 slot, autonomous, relay-independent**)

**Trigger**: relay availability irrelevant; never depends on OneAPI state.

**Goal**: capture R85 + R86 dogfood live-protocol JSONL transcripts during a green-relay window (already exist after R85 + R87), build a fake `claude-agent-sdk` shim that replays the captured envelopes, and assert recorder + fork primitive correctness against the fake. Result: AC-3 gate becomes deterministic and runs in CI without `CHRONOS_LIVE=1` or `ANTHROPIC_API_KEY`. Recurring relay flakes (R86 surface, R69 surface) no longer block the GA gate.

**Plan**:
1. **Capture** — re-run R85 MCP + R86 fork-override dogfoods with `CHRONOS_PROTOCOL_LOG=/tmp/r88_*.jsonl` (need to wire env-var passthrough into the dogfoods if not already there; ~10 LOC each).
2. **Fake SDK shim** — `tests/fakes/anthropic_agents_replay.py`: monkeypatchable wrapper that intercepts `claude_agent_sdk.query(...)` calls and yields recorded envelopes by `(session_id, message_index)` lookup. Same shape for `fork_session(up_to_message_id=...)` — return a frozen `child_session_id` from the captured fork response.
3. **ADR-027 Draft** — `docs/decisions/ADR-027-anthropic-replay-seam.md`: scope = "the seam between adapter and SDK is JSONL-replayable", contract surface, replay-vs-record fidelity invariants. Mark Status: Proposed.
4. **Tests** — `tests/unit/test_anthropic_agents_offline_fixture.py`: 5 invariants mirroring R86 dogfood, but driven by replay shim. Strict-xfail until shim wired.
5. **Document** — `docs/adapters/anthropic_agents.md` "Offline testing" section.

**Pre-budget**: 1 slot if R85+R87 protocol logs are already captured (verify in `/workspace/chronos-agent/protocol_logs/` first); 2 slots if recapture needed.

**Risk**: low — purely additive; touches `tests/fakes/` and new ADR; no src/ change. Strict-xfail forcing function applies.

### Option C — recorder kind contract reconciliation (light, **1 slot**)

**Trigger**: chosen if Option β protocol logs not preserved or relay needed to recapture.

**Goal**: address R85 contract finding "ToolUseBlock and ToolResultBlock surface as `kind=NodeKind.LLM`" (kind from message-type, not block-type). Two valid resolutions:

- **(a) Lightweight**: document "envelope-determines-kind" in `docs/contracts/adapter-protocol.md` (AdapterProtocol contract docs), plus a 1-line note in `docs/adapters/anthropic_agents.md`. ~30 LOC delta. Recommended for R88.
- **(b) Heavy**: split per-block nodes refactor — would change `kind` semantics across all 4 adapters. ADR-deserving (ADR-028 candidate). Defer to Phase 5 if needed.

**Pre-budget**: 0.5-1 slot for (a). Single-slot impl + docs round.

**Risk**: minimal — documentation only.

### Option D — 6th fixture site migration (mechanical, **0.5-1 slot**)

**Trigger**: low-risk warm-up round if Options β/C deferred.
...
**Trigger**: chosen if R88 wants a planning round vs an impl round.

**Goal**: re-read `docs/roadmap.md` for Arc selection. Phase 5 candidates may include: Arc C (a different SDK adapter — OpenAI Agents SDK? Vercel AI SDK?), recorder-seam refactor (split kinds per Option Cb), web frontend feature batch (R37/R46-A modal pattern extension to fork plan editing UX), or cross-adapter compare (R65 N-run compare extension to "compare across adapter implementations of same agent loop"). Author `docs/research/r88-phase5-survey.md` + `docs/decisions/ADR-028-phase5-arc-selection.md` Draft.

**Pre-budget**: 1-2 slots; md-only zero gate-drift.

**Risk**: minimal — planning round, no code.

### Hard constraints / process invariants R88 must honor

- **Disprover-first** (R73 invariant, formalized R87): if R88 picks any option that depends on a previous-round un-tested research conclusion (e.g., "R85 protocol log was captured cleanly"), run smallest disprover before committing budget.
- **2-slot pre-budget for impl rounds** (12-round inheritance chain R48-A → R86, structural constant): if R88 picks β/C-b, budget 2 slots with explicit slot-1 / slot-2 plan.
...
- **Strict-xfail forcing function** (R76→R77 / R79→R80 / R81→R82 pattern): if R88 picks β, write the 5 invariants as strict-xfail tests against the not-yet-existing fake shim, then implement until all 5 turn green. R87 confirmed this pattern shipped 3 of 5 Arc B impl rounds at green-on-first-iteration.
- **No retroactive AC unratchet on relay flake** (R86 invariant codified at R87): if R88 hits a relay flake, AC-3 stays `[x]` (R87's recorded green run is a time-stamped ratchet). Only a hard regression in adapter code unratchets a recorded AC.
- **Aspirational-release-doc trap detector** (`cron-slot-handoff-recovery`): if R88 inherits a WIP that claims a release block, run trap-shape diagnostic (un-bumped pyproject + no progress doc + no tag → revert + re-run live gate before any tag).

### What's done (no need to redo at R88)
...
- ✅ R86 dogfood scaffolding shipped (production-grade, no rebuild needed at R88).
- ✅ Arc B slice 1 implementation series complete (R70-R83 alpha, R85-R87 GA close).
- ✅ Adapter-1-3 zero-regression streak R52→R87 = 35 rounds (continuing).
- ✅ R86 contract pre-finding promoted to finding at R87 (live observation matched source-inspection prediction).

### Cost outlook for R88

- Option β: $0 (offline) for impl + $0.20 if recapture needed.
- Option C: $0 (docs only).
- Option D: $0 (test refactor only).
- Option ε: $0 (planning only).

### v0.7.0+ release version line

- v0.7.0 ✅ shipped at R87 (Arc B slice 1 GA, all 5 ACs, AC-3 closed against live relay).
- v0.7.1 — patch candidate: Option C(a) docs + Option D fixture migration (~1-2 slots bundled). Tag once both ship cleanly. No semver-public-surface delta.
